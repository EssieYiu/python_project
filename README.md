...
6. 重新修改了user-merchant_and_coupon_feature中的coupon的特征提取，取的是dataset的数据，自带有date项（dataset1，2中有，dataset3中没有，drop的时候因此不需要drop dataset3的date），因此，取消掉了原来从user-merchant中获取date项的列(避免重复)
7. 所有的dataset都drop掉了"Discount_rate"列，原因是计算出的折扣率存放在discount_rate列中了，以小数形式表示

## 运行方法
运行user_and_merchant_feature.py获取user特征和merchant特征，运行user-merchant_and_coupon_feature.py获取用户-商家交互特征以及优惠券特征，运行other_feature.py获取其他特征。运行generate.py整合数据，运行xgb.py进行模型训练以及预测